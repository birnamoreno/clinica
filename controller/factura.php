...
class FacturaController
{
    ...
    public static function ctrMostrarFactura()
    {
        ...
        if (isset($_GET["folio"])) {
            ...
            $stmt = Conexion::conectar()->prepare("SELECT t1.* , t2.folio_cita , CONCAT_WS(' ' , t3.nombre_paciente , t3.apaterno_paciente , t3.amaterno_paciente) AS nombre_paciente,
                t3.correo_paciente, t3.telefono_paciente, CONCAT_WS(' ' , t4.nombre_medico , t4.apaterno_medico , t4.amaterno_medico) AS nombre_medico 
                FROM tbl_pago t1 
                ...
                WHERE t1.folio_pago = :folio");
            $stmt->bindParam(":folio", $_GET["folio"], PDO::PARAM_STR);
            $stmt->execute();
            $response = $stmt->fetch();
        } else {
    ...
    public static function ctrAgregarFactura($datos)
    {
    ...
    public static function ctrActulizarFactura($datos)
    {
...
switch ($_SERVER["REQUEST_METHOD"]) {
    case 'GET':
        FacturaController::ctrMostrarFactura();
        break;
    case 'POST':
        $datos = json_decode(file_get_contents('php://input'));
        FacturaController::ctrAgregarFactura($datos);
        break;
    case 'PUT':
        $datos = json_decode(file_get_contents('php://input'));
        FacturaController::ctrActulizarFactura($datos);
        break;
    case 'DELETE':
        FacturaController::eliminarPago();
        break;
    default:
